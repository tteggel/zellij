...
pub struct ShaderUniforms {
    ...
    pub cursor_x: i32,
    pub cursor_y: i32,
    pub mouse_x: i32,       // -1 if not hovering
    pub mouse_y: i32,
    pub time_ms: i32,
    pub pane_id: i32,
    pub is_focused: i32,     // 0 or 1
...
pub struct ShaderInstance {
    ...
    invalidate_fn: Option<wasmi::TypedFunc<(i32, i32), i32>>,
}
...
impl ShaderInstance {
    pub fn new(wasm_bytes: &[u8]) -> Result<Self, String> {
        ...
        let invalidate_fn = instance
            .get_typed_func::<(i32, i32), i32>(&store, "invalidate")
            .ok();
        Ok(Self {
            engine,
    ...
    // at low addresses. 128 bytes (65408..65536), just below invalidation output.
    ...
    pub fn invalidate(
        ...
        let invalidate_fn = match self.invalidate_fn {
            Some(ref f) => f,
            None => return Vec::new(),
        };
        ...
        // Use offset after shade_batch data area
        let out_offset: usize = 65536;
        let max_rows = uniforms.pane_height;
        let needed = out_offset + max_rows as usize * 4;
        let current_size = self.memory.data_size(&*store);
        if needed > current_size {
            let pages = ((needed - current_size + 65535) / 65536) as u32;
            let _ = self.memory.grow(&mut *store, pages.into());
        ...
        // Write uniforms to WASM memory
        self.write_uniforms(&mut *store, uniforms);
        let count = invalidate_fn.call(
            &mut *store,
            (out_offset as i32, max_rows as i32),
        ).unwrap_or(0);
        if count <= 0 {
            return Vec::new();
        }
        ...
        for i in 0..count as usize {
            let base = out_offset + i * 4;
            if let Ok(bytes) = mem_data[base..base+4].try_into() {
                rows.push(i32::from_le_bytes(bytes) as usize);
            }
        }
        rows
    }
